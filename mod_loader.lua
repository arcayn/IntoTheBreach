...
function mod_loader:init()
	...
	for i, id in ipairs(orderedMods) do	
		...
		if satis then
			self:initMod(id)
			self.mods[id].compat = true
		else
			self.mods[id].compat = false
			local stillReq = ""
			for j,w in pairs(newReqs) do
				LOG(string.format("Please install mod with id [%s] to ensure compatibility with mod %s",j, self.mods[id].name))
			end
